...
export default function BursarRecordPayment() {
    ...
    useEffect(() => {
        const fetchResults = async () => {
            ...
            try {
                const res = await searchStudents({ q: searchQuery })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setForm({
                studentId: '', studentName: '', admissionNumber: '', amount: '',
                paymentMethod: 'cash', paymentDate: new Date().toISOString().split('T')[0],
                session: `${new Date().getFullYear()}/${new Date().getFullYear() + 1}`,
                term: 'First Term', feeType: 'tuition', description: '',
            })
            setSearchQuery('')
        } catch (err) {
    ...
    return (
        <main className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
            <div className="mb-8">
                ...
                <p className="mt-3 text-lg text-slate-400">Record cash, bank transfer, POS, or online payments.</p>
            </div>

            <form onSubmit={handleSubmit} className="rounded-[2rem] border border-slate-800 bg-slate-900/80 p-6 sm:p-8">
                {error && <div className="mb-6 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-300">{error}</div>}
                {success && <div className="mb-6 rounded-2xl border border-green-500/30 bg-green-500/10 px-4 py-3 text-sm text-green-300">{success}</div>}
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <label className="block text-sm text-slate-400">
                                ...
                                <div className="relative">
                                    <input type="text" value={searchQuery}
                                        onChange={(e) => { setSearchQuery(e.target.value); setForm((prev) => ({ ...prev, studentId: '', admissionNumber: '' })) }}
                                        ...
                                        placeholder="Search by name or admission number" required />
                                    {showDropdown && (
                                        <div className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-2xl border border-slate-700 bg-slate-900 shadow-xl">
                                            {searchResults.map((student) => (
                                                <button key={student._id} type="button" onMouseDown={() => selectStudent(student)}
                            ...
                            <label className="block text-sm text-slate-400">
                                ...
                                <input type="text" value={form.admissionNumber} readOnly
                                    className="w-full rounded-full border border-slate-700 bg-slate-800 px-4 py-3 text-slate-300" placeholder="Auto-filled" />
                            </label>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <label className="block text-sm text-slate-400">
                                ...
                                <input type="number" name="amount" value={form.amount} onChange={handleChange} min="1" required
                                    className="w-full rounded-full border border-slate-700 bg-slate-950 px-4 py-3 text-white" placeholder="100000" />
                            </label>
                            <label className="block text-sm text-slate-400">
                            ...
                            <label className="block text-sm text-slate-400">
                                ...
                                <input type="date" name="paymentDate" value={form.paymentDate} onChange={handleChange}
                                    className="w-full rounded-full border border-slate-700 bg-slate-950 px-4 py-3 text-white" required />
                            </label>
                            <label className="block text-sm text-slate-400">
                    ...
                    <div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <label className="block text-sm text-slate-400">
                                ...
                                <input type="text" name="session" value={form.session} onChange={handleChange}
                                    className="w-full rounded-full border border-slate-700 bg-slate-950 px-4 py-3 text-white" required />
                            </label>
                            <label className="block text-sm text-slate-400">
                    ...
                    {form.paymentMethod === 'cash' && (
                        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
                            A unique cash receipt number will be automatically generated.
                        </div>
                    )}
