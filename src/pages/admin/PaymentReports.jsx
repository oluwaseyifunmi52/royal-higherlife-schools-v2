...
const methods = ['', 'cash', 'bank_transfer', 'pos', 'paystack', 'flutterwave', 'other']
...
export default function PaymentReports() {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const res = await api.get('/api/payments/reports', { params })
  ...
  const handleExportCSV = () => {
    ...
    const rows = payments.map((p) => [
      ...
      feeTypeLabels[p.feeType] || p.feeType,
      p.term,
      p.session,
    ])
    ...
    <main className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="mt-3 text-lg text-slate-400">Generate and export detailed payment reports.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={handlePrint} className="rounded-full border border-slate-700 px-6 py-3 font-semibold text-white">Print</button>
      ...
      <div className="rounded-[2rem] border border-slate-800 bg-slate-900/80 p-6 mb-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          <label className="block text-sm text-slate-400">
            ...
            <input type="date" name="startDate" value={filters.startDate} onChange={handleFilterChange} className="w-full rounded-full border border-slate-700 bg-slate-950 px-4 py-3 text-white" />
          </label>
          <label className="block text-sm text-slate-400">
            <span className="mb-2 block">End Date</span>
            <input type="date" name="endDate" value={filters.endDate} onChange={handleFilterChange} className="w-full rounded-full border border-slate-700 bg-slate-950 px-4 py-3 text-white" />
          </label>
          <label className="block text-sm text-slate-400">
            <span className="mb-2 block">Payment Method</span>
            <select name="paymentMethod" value={filters.paymentMethod} onChange={handleFilterChange} className="w-full rounded-full border border-slate-700 bg-slate-950 px-4 py-3 text-white">
          ...
          <label className="block text-sm text-slate-400">
            ...
            <select name="term" value={filters.term} onChange={handleFilterChange} className="w-full rounded-full border border-slate-700 bg-slate-950 px-4 py-3 text-white">
          ...
          <label className="block text-sm text-slate-400">
            ...
            <input type="text" name="session" value={filters.session} onChange={handleFilterChange} className="w-full rounded-full border border-slate-700 bg-slate-950 px-4 py-3 text-white" placeholder="2026/2027" />
          </label>
        </div>
        <button onClick={fetchReport} disabled={loading} className="mt-4 rounded-full bg-amber-500 px-6 py-3 font-semibold text-slate-950 disabled:opacity-50">
              ...
              <tbody>
                {payments.map((p) => (
                  <tr key={p._id} className="border-t border-slate-800 text-slate-300">
                    ...
                    <td className="px-4 py-3">{feeTypeLabels[p.feeType] || p.feeType}</td>
